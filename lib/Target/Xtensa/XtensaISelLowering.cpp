...
const char *XtensaTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  default:
    return NULL;
  case XtensaISD::RET_FLAG: return "RetFlag";
  case XtensaISD::LOAD_SYM: return "LOAD_SYM";
  case XtensaISD::MOVEi32:  return "MOVEi32";
  case XtensaISD::CALL:     return "CALL";
  }
}

XtensaTargetLowering::XtensaTargetLowering(XtensaTargetMachine &XtensaTM)
    : TargetLowering(XtensaTM), Subtarget(*XtensaTM.getSubtargetImpl()) {
  // Set up the register classes.
  addRegisterClass(MVT::i32, &Xtensa::GRRegsRegClass);
...
SDValue XtensaTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
SDValue XtensaTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG& DAG) const
{
  EVT VT = Op.getValueType();
  GlobalAddressSDNode *GlobalAddr = cast<GlobalAddressSDNode>(Op.getNode());
  SDValue TargetAddr =
      DAG.getTargetGlobalAddress(GlobalAddr->getGlobal(), Op, MVT::i32);
  return DAG.getNode(XtensaISD::LOAD_SYM, Op, VT, TargetAddr);
}
...
/// Xtensa call implementation
SDValue XtensaTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Xtensa);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    SDValue StackPtr = DAG.getRegister(Xtensa::A1, MVT::i32);
  ...
  Chain = DAG.getNode(XtensaISD::CALL, Loc, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
SDValue XtensaTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Xtensa);
...
/// Xtensa formal arguments implementation
SDValue XtensaTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_Xtensa);
  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      const unsigned VReg = RegInfo.createVirtualRegister(&Xtensa::GRRegsRegClass);
...
bool XtensaTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_Xtensa)) {
    return false;
  }
...
XtensaTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Xtensa);
